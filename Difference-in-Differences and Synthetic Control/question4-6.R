### Question 4 - Part 6: main estimation and inference ###
...
# Placebo group: mothers with youngest child >5 and <17 years old
placebo_list <- list()

for (var in outcomes){
  placebo_list[[var]] <- FP_estimation(
    outcome = var,
    treatdum = 'treat',
    postdum = 'post', 
    timevar = 'YEAR',
    groupvar = 'STATEFIP',
    data = dt[!is.na(get(var)) & YNGCH > 5 & YNGCH < 17],
    controls = c('age_group', 'race_group', 'educ_group'), 
    weightvar = 'ASECWT',
    return_lean = TRUE
  )
}
...
group <- rep(c('DiD estimate', '95% CI', 'p-value', ' '), 4)
...
for (var in outcomes){
  out[[var]] <- list(
    ...
    round(placebo_list[[var]]$coefficient, 3), 
    paste0('[', round(placebo_list[[var]]$ci.low, 3), '; ',
           round(placebo_list[[var]]$ci.high, 3), ']'),
    round(placebo_list[[var]]$pvalue, 3),
    paste0('N = ', placebo_list[[var]]$model$nobs)
  )
}
...
kbl(data.table(group, out[[1]], out[[2]], out[[3]], out[[4]], out[[5]]), 
    ...
    booktabs = TRUE,
    digits = 3,
    caption = "DiD estimates of impact of California's PFL program on labor
  ...
  pack_rows('Placebo group: mothers with youngest child aged between 5 and 17 years old',
             13, 16) %>%
  landscape() %>%
  save_kable('Output/main_DiD.tex')
